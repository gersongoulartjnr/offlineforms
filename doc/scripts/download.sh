...
pcts_download(){
	...
	if [ -e "cassandra.tgz" ] ; then
		...
		wget -O cassandra.tgz https://archive.apache.org/dist/cassandra/1.2.19/apache-cassandra-1.2.19-bin.tar.gz >> debug_download.txt
	fi

	if [ -e "tomcat7.tgz" ] ; then
		echo "Tomcat Já Baixado"
	else	
		wget -O tomcat7.tgz http://archive.apache.org/dist/tomcat/tomcat-7/v7.0.61/bin/apache-tomcat-7.0.61.tar.gz >> debug_download.txt
	fi
